...
class Uploader
{
    ...
    protected function process_archive()
    {
        ...
        if ($res === true) {
            ...
            while ($filename = readdir($dir)) {
                if ($filename == "." || $filename == ".." || !is_file($this->folder . '/' . $filename)) {
                    continue;
                }
    ...
    {
        foreach ($this->getImages() as $image_file => $name) {
            $this->addImageToAlbum($image_file, $name, $album, $options, $user);
        }
        if ($this->getUploadedFolder()) {
            exec(sprintf('rm -rf %s', $this->getUploadedFolder()));
        }
    }
     ...
     * @param UserInterface $user
     */
    private function addImageToAlbum(
        $image_file,
        $default_title,
        ...
        $image->setFilename(
            sprintf("%s.%s", md5(uniqid()), strtolower(pathinfo($default_title ?: $image_file, PATHINFO_EXTENSION)))
        );
