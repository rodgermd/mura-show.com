...
class RenameFoldersCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($repo->findAll() as $album) {
            /** @var Album $album */
            $output->writeln(
                sprintf("<info>[%s]</info>\t%s => %s", $album->getName(), $album->getSlug(), $album->getId())
            );
            $finder = new Finder();
            ...
            $finder = new Finder();
            foreach ($finder->directories()->name($album->getSlug())->in(
                $this->getContainer()->getParameter('kernel.root_dir') . '/../uploads'
            ) as $dir) {
                $output->writeln($dir->getRealpath());
                $this->addResult($album, $dir);
            }
